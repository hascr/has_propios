...
class AccountInvoiceElectronic(models.Model):
    ...
    type_override = fields.Selection(TYPE_SELECTION, string="Type Override")
    ...
    state_invoice_partner = fields.Selection(
        selection=[("1", "Aceptado"), ("2", "Aceptación parcial"), ("3", "Rechazado")],
    ...
    qr_image = fields.Binary(string="QR Code", compute="_compute_qr_code")
    ...
    error_count = fields.Integer(string="Número de errores", default="0", copy=False)
        ...
        for record in self:
            ...
            if self.env.user.company_id.invoice_qr_type != "by_info":
                ...
                if self.env.user.company_id.invoice_field_ids:
                    dict_result = {}
                    for ffild in self.env.user.company_id.invoice_field_ids.mapped(
                        "field_id"
                    ):
    ...
    @api.constrains("payment_methods_id")
    def check_dev_tax(self):
        self.update_dev_tax()
            ...
            if inv.move_type in ("in_invoice", "in_refund"):
                if inv.partner_id:
                    inv.economic_activities_ids = (
                        inv.partner_id.economic_activities_ids
    ...
    def get_invoice_sequence(self):
        ...
        sequence = False
        no_sequence_message = (
            "This journal doesn't have the sequence configured for documents of type: "
            + tipo_documento
            + ". Please consider to configure the sequence and reset the invoice to draft."
        )
        ...
        if self.move_type == "out_invoice":
            ...
            if (
                ...
                and not self.partner_id.property_account_position_id
            ):
                raise UserError(
                    _(
                        "Indique la cédula, el tipo de identificación y la posición fiscal al cliente relacionado."
                    )
                )
            ...
            try:
                ...
                if estado_m_h == "aceptado":
                    ...
                    decoded_xml = base64.b64decode(
                        response_json.get("respuesta-xml")
                    ).decode("utf-8")
                    xml_errors = decoded_xml.partition("<DetalleMensaje>")[2].partition(
                        "</DetalleMensaje>"
                    )[0]
                    ...
                    _logger.error(
                        "E-INV CR - Query Hacienda - Invoice not found: %s  -  "
                        "Hacienda Status: %s",
                        i.number_electronic,
                        estado_m_h,
                    )
                    ...
                    if (
                        ...
                        if (
                            ...
                            if inv.state_tributacion in ["rechazado", "error"]:
                                ...
                                message_description += _(
                                    "Consecutive Switching of Receiver Message</b><br/>"
                                )
                            ...
                            if inv.state_tributacion != "procesando":
                                ...
                                if inv.state_tributacion == "procesando":
                                    ...
                                    if status == 200:
                                        ...
                                        message_description += (
                                            _(
                                                "<p><b>Ha enviado Mensaje de Receptor</b>"
                                            )
                                            + _("<br /><b>Documento: </b>")
                                            + inv.number_electronic
    ...
    def _send_invoices_to_hacienda(self, max_invoices=10):  # cron
        ...
        _logger.info("E-INV CR - days_left: %s", days_left)
        invoices = self.env["account.move"].search(
            [
                ("move_type", "in", ["out_invoice", "out_refund"]),
                ("state", "=", "posted"),
        ...
        _logger.info("E-INV CR - Invoices to send: %s", len(invoices))
        if days_left >= 0:
    ...
    def action_send_mrs_to_hacienda(self):
        if self.state_invoice_partner:
            if self.state_tributacion not in ["procesando"]:
                self.state_tributacion = False
    ...
    def generate_and_send_invoice(self):
        _logger.info("generate_and_send_invoice")
        days_left = self.env.user.company_id.get_days_left()
        if days_left >= 0:
            self.generate_and_send_invoices(self)
        else:
    ...
    def generate_and_send_invoices(self, invoices):
        ...
        for inv in invoices:
            try:
                ...
                if (
                    not inv.sequence or not inv.sequence.isdigit()
                ):  # or (len(inv.number) == 10):
                ...
                if not inv.xml_comprobante or (
                    ...
                    for inv_line in inv.invoice_line_ids.filtered(
                        ...
                        env_iva_devuelto = self.env.ref(
                            "cr_electronic_invoice.product_iva_devuelto"
                        ).id
                        if (
                            inv_line.product_id
                            and inv_line.product_id.id == env_iva_devuelto
                        ):
                            ...
                            if inv_line.discount and price_unit > 0:
                                ...
                                line["naturalezaDescuento"] = (
                                    inv_line.discount_note or "Descuento Comercial"
                                )
                            ...
                            if inv_line.tax_ids:
                                ...
                                for i in line_taxes["taxes"]:
                                    ...
                                    elif taxes_lookup[i["id"]]["tax_code"] not in [
                                        "00",
                                        "02",
                                        "99",
                                        "DEV",
                            ...
                            if (
                                not inv_line.product_uom_id
                                or inv_line.product_uom_id.category_id.name
                                in (
                                    "Service",
                                    "Services",
                                    "Servicio",
                    ...
                    for line in inv.line_ids:
                        if line.tax_line_id and line.tax_line_id.tax_group_id.name in [
                            "911",
                            "Cruz Roja",
                        ]:
                            otros_cargos_id += 1
                            otros_cargos[otros_cargos_id] = {
                                "TipoDocumento": line.tax_line_id.tax_code,
                                "Detalle": escape(line.tax_line_id.description[:150]),
                                "MontoCargo": line.price_subtotal,
                            }
                            if line.third_party_id:
                                otros_cargos[otros_cargos_id][
                                    "NombreTercero"
                                ] = line.third_party_id.name

                                if line.third_party_id.vat:
                                    otros_cargos[otros_cargos_id][
                                        "NumeroIdentidadTercero"
                                    ] = line.third_party_id.vat

                            total_otros_cargos += abs(line.balance)
                        if line.tax_line_id and line.tax_line_id.iva_tax_code in [
                            "DEV04"
                        ]:
                            total_iva_devuelto += abs(line.balance)
                ...
                if 200 <= response_status <= 299:
                    ...
                    if response_text.find("ya fue recibido anteriormente") != -1:
                        ...
                        _logger.error(
                            _(
                                f"E-INV CR  - Invoice: {inv.number_electronic}"
                        ...
                        _logger.error(
                            _(
                                "E-INV CR  - Invoice: %s  Status: %s Error "
                                "sending XML: %s"
                                % (
                                    inv.number_electronic,
                                    response_status,
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if (
                not vals.get("payment_methods_id")
                and partner
                and partner.payment_methods_id
            ):
    ...
    def action_post(self):
        ...
        for inv in self:
            ...
            if not inv.company_id.vat:
                raise UserError(
                    _("Please configure the identification in the company profile")
                )
            if (
                not inv.company_id.state_id
                or not inv.company_id.county_id
                or not inv.company_id.district_id
                or not inv.company_id.neighborhood_id
                or not inv.company_id.street
            ):
                raise UserError(
                    _("Please complete the address information in the company profile")
                )
            ...
            if not inv.number_electronic:
                ...
                sucursal_id = (
                    inv.journal_id.sucursal or self.env.user.company_id.sucursal_MR
                )

                # if journal doesn't have terminal use default from company
                terminal_id = (
                    inv.journal_id.terminal or self.env.user.company_id.terminal_MR
                )
    ...
    def update_dev_tax(self):
        for record in self:
            for line in record.invoice_line_ids:
                for tax in line.tax_ids:
                    if (
                        record.payment_methods_id
                        and record.payment_methods_id.sequence == "02"
                    ):
                        line.tax_ids += tax.dev_tax_id
                    else:
                        line.write({"tax_ids": [(3, tax.dev_tax_id.id)]})
                        # line.line -= tax.dev_tax_id
        self._sync_dynamic_lines(self)
    ...
    def action_invoice_sent_mass(self):
        if self.invoice_id.move_type in ["in_invoice", "in_refund"]:
            email_template = self.env.ref(
                "cr_electronic_invoice.email_template_invoice_vendor",
        ...
        if self.env.user.company_id.frm_ws_ambiente == "disabled":
            ...
            if attachment:
                ...
                if attachment_resp:
                    ...
                    _logger.error("E-INV CR - MASS SEND - Exitoso: %s", self.sequence)
                    email_template.attachment_ids = [(5, 0, 0)]
    ...
    @api.onchange("name", "partner_id", "invoice_date", "tipo_documento")
