            ...
            if inv.xml_supplier_approval:
                ...
                if inv.state_tributacion == 'procesando':
                    ...
                    if inv.company_id.frm_ws_ambiente != 'disabled' and inv.state_invoice_partner:
                        ...
                        if not inv.xml_comprobante or inv.state_invoice_partner not in ['1', '2']:
            ...
            try:
                ...
                if not inv.xml_comprobante or (inv.tipo_documento == 'FEC' and inv.state_tributacion == 'rechazado'):
                        ...
                        if inv_line.product_id and inv_line.product_id.id == env_iva_devuelto:
                            ...
                            if inv_line.tax_ids:
                                ...
                                for i in line_taxes['taxes']:
                                    ...
                                    elif taxes_lookup[i['id']]['tax_code'] not in ['00', '02', '99', 'DEV']:
                                        tax_index += 1
                    ...
                    for line in inv.line_ids:
                        if line.tax_line_id and line.tax_line_id.tax_group_id.name in ['911', 'Cruz Roja']:
                            otros_cargos_id += 1
                            otros_cargos[otros_cargos_id] = {
                            ...
                            if line.third_party_id:
                                otros_cargos[otros_cargos_id]['NombreTercero'] = line.third_party_id.name
                        ...
                        if line.tax_line_id and line.tax_line_id.iva_tax_code in ['DEV04']:
        ...
        for record in self:
            for line in record.invoice_line_ids:
                for tax in line.tax_ids:
                    if record.payment_methods_id and record.payment_methods_id.sequence == '02':
                        line.tax_ids += tax.dev_tax_id
                    else:
                        line.write({
                            'tax_ids': [(3, tax.dev_tax_id.id)]
                        })
    ...
    def action_send_and_print(self):
        ...
        if attachment:
            ...
            if attachment_resp:
                ...
                attach_copy = attachment.copy()
                attach_resp_copy = attachment_resp.copy()
                template.attachment_ids = [(6, 0, [attach_copy.id, attach_resp_copy.id])]
                ##email_template.attachment_ids = [(6, 0, [attachment.id, attachment_resp.id])]
            else:
