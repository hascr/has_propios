    ...
    def init(self):
        self._cr.execute("""
            ...
            SELECT		s.id AS id,
				...
				(SELECT a."value"->> 'es_CR' FROM survey_question_answer a WHERE a.id = (SELECT MAX(l.suggested_answer_id) filter (WHERE l.question_id = 114) FROM	survey_user_input_line l WHERE l.user_input_id = s.id)) AS area_trabajo
